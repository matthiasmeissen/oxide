...
fn main() {
    ...
    // Triple Buffer for outgoing state
    let (mut state_write, mut state_reader) = TripleBuffer::new(&State::default()).split();
    ...
    thread::spawn(move || {
        ...
        while let Ok(update) = receiver.recv() {
            ...
            state_write.write(current_state);
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            midi_sender.try_send(Message::SetValue(0, val)).ok();
