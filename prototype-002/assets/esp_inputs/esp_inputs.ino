// Wiring Diagram ESP32 to Raspberry PI 5

// Board Pins
// GND(BLK) - SCL(BLU) - SDA(GRE) - VCC(RED)
...
void loop() {
  ...
  // This inverts the pot readings, when vcc and gnd are wired the wrong way
  // To do a regular reading do: stablePotValue1 / 4
  // Also the / 4 is intended to make the value more stable
  localState.pot1_value = 1023 - (stablePotValue1 / 4);

  uint16_t stablePotValue2 = pot2.read();
  localState.pot2_value = 1023 - (stablePotValue2 / 4);

  uint16_t stablePotValue3 = pot3.read();
  localState.pot3_value = 1023 - (stablePotValue3 / 4);

  uint16_t stablePotValue4 = pot4.read();
  localState.pot4_value = 1023 - (stablePotValue4 / 4);
