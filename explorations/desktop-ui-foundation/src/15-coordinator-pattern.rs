...
fn main() {
    ...
    let coordinator_state = shared_state.clone();
    thread::spawn(move || {
        ...
        while let Ok(update) = receiver.recv() {
            ...
            *coordinator_state.write().unwrap() = current_state;
            print!("// -> Cooridnator update state: {:#?}", current_state);
        }
    ...
    thread::spawn(move || {
        let mut val = 0.0;
        loop {
            midi_sender.send(Message::SetValue(0, val)).unwrap();
            println!("MIDI update value: {val}");
            val += 1.0;
    ...
    thread::spawn(move || {
        ...
        loop {
            // Send Time Value
            window_sender.send(Message::SetTime(time)).unwrap();
            println!("Window update time: {time}");

            // Read State
            let state = *window_state.read().unwrap();
