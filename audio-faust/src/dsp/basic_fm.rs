...
impl BasicFm {
	...
	pub fn build_user_interface_static(ui_interface: &mut dyn UI<FaustFloat>) {
		...
		ui_interface.add_horizontal_slider("mod_value", ParamIndex(1), 4e+01, 2e+01, 8e+01, 0.1);
	...
	pub fn compute(
		...
		let mut fSlow1: F32 = self.fConst0 * self.fHslider1;
		...
		for (output0, output1) in zipped_iterators {
			...
			let mut fTemp1: F32 = (if iTemp0 != 0 {0.0} else {fSlow0 + self.fRec2[1]});
			self.fRec2[0] = fTemp1 - F32::floor(fTemp1);
			let mut fTemp2: F32 = (if iTemp0 != 0 {0.0} else {self.fRec1[1] + fSlow1 * ftbl0mydspSIG0_guard[(std::cmp::max(0, std::cmp::min((65536.0 * self.fRec2[0]) as i32, 65535))) as usize]});
			self.fRec1[0] = fTemp2 - F32::floor(fTemp2);
			let mut fTemp3: F32 = 0.2 * ftbl0mydspSIG0_guard[(std::cmp::max(0, std::cmp::min((65536.0 * self.fRec1[0]) as i32, 65535))) as usize];
			*output0 = fTemp3;
			*output1 = fTemp3;
